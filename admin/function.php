...
if(isset($_POST['btn_hapus_kategori'])){
  ...
  // sql to delete a record
  $sql_hapus_artikel = "DELETE FROM kategori WHERE id_kategori = '$id_hapus'";
...
if(isset($_POST['btn_ubah_penulis'])){
  ...
  $sql = "UPDATE penulis SET nama_penulis='$data_nama', email = '$data_email', password = '$data_password' 
...
if(isset($_POST['btn_hapus_penulis'])){
  ...
  // sql to delete a record
  $sql_hapus_artikel = "DELETE FROM penulis WHERE id_penulis = '$id_hapus'";
...
function potong_artikel($paragraf, $jml_karakter){
  // Ensure the number of characters is within bounds
  if ($jml_karakter >= strlen($paragraf)) {
    return $paragraf; // No need to cut if the requested length is greater than the paragraph length
  }
  
  // Move backwards to find a space character
  while($jml_karakter > 0 && $paragraf[$jml_karakter] != " "){
    --$jml_karakter;
  }
  
  // If no space found, cut at the requested character limit
  if ($jml_karakter == 0) {
    $jml_karakter = strlen($paragraf);
  }
  
  $potongan_paragraf = substr($paragraf, 0, $jml_karakter);
  $potong_paragraf_jadi = $potongan_paragraf . "...";
  
  return $potong_paragraf_jadi;
}
